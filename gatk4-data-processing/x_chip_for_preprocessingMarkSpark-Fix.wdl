...
# Aggregate aligned+merged flowcell BAM files and mark duplicates
  ...
  call MarkDuplicatesSparkandFix {
    input:
      TI = 0,
  ...
  call TS as S2{
	input:
		...
		I = MarkDuplicatesSparkandFix.T
  }
  # Sort aggregated+deduped BAM file and fix tags
  output {
    File duplication_metrics = MarkDuplicatesSparkandFix.duplicate_metrics
  } 
}
...
task SamToFastqAndBwaMem {
  ...
  command <<<
    ...
    > tmp.txt
		(time (cat tmp.txt | (samtools view -1 - > ${output_bam_basename}.bam))) 2> "/arc-local/gatk/LocalTool/SamtoolsTime.txt"
...
task MarkDuplicatesSparkandFix {
 ...
 # While query-grouped isn't actually query-sorted, it's good enough for MarkDuplicates with ASSUME_SORT_ORDER="queryname"
  command {
	...
	sudo time -ap -o "/arc-local/gatk/LocalTool/surfaceTime/markduplicatesspark.txt" \
    ${gatk_path} --java-options "-Dsamjdk.compression_level=${compression_level} ${java_opt}" \
      MarkDuplicatesSpark \
      -I ${sep=' -I ' input_bams} \
      -O tmp.txt \
